...
:build_one_project
echo "Build %2 => %3 in release mode for %1"
DevEnv %2 /build release /project %3
if errorlevel 1 (
   echo Failure build %2=>%3 in release mode. Return code is %errorlevel%
   exit /b %errorlevel%
)

:build_all_projects
echo "Build all projects from %2 in release mode for %1"
DevEnv %2 /build release
if errorlevel 1 (
   echo Failure build %2 in release mode. Return code is %errorlevel%
   exit /b %errorlevel%
)
goto :eof
